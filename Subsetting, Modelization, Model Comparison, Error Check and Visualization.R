...
# Arranging by Rsquared to see the best models
PerformanceMetrics %>% 
  arrange(desc(Rsquared)) # Random Forest models perform the best!

AccuracyMetrics %>% 
  arrange(desc(Accuracy)) # Random Forest models perform the best!

### ---- RF Error Check ----
# Creating data frames for random forest predictions
RFdata0 <- data.frame(rf.longitude = predRF0Long, 
                      rf.latitude = predRF0Lat, 
                      rf.floor = predRF0Floor)
  
RFdata1 <- data.frame(rf.longitude = predRF1Long, 
                      rf.latitude = predRF1Lat, 
                      rf.floor = predRF1Floor)

RFdata2 <- data.frame(rf.longitude = predRF2Long, 
                      rf.latitude = predRF2Lat, 
                      rf.floor = predRF2Floor)

# Combining the data frames
RFdataErr <- rbind(RFdata0, RFdata1, RFdata2)

RFdataErrValid <- cbind(RFdataErr, 
...
RFdataErrValid %>% 
  ggplot(aes(x = WifiValidationData$LONGITUDE, y = WifiValidationData$LATITUDE)) +
  geom_point(aes(x = WifiValidationData$LONGITUDE, 
                 y = WifiValidationData$LATITUDE), 
             color = "red") +
  geom_point(aes(x = rf.longitude, 
                 y = rf.latitude), 
             color = "blue") +
